...
function handleProgramHashScroll() {
    ...
    if (!target) {
        ...
        const mappedId = aliasMap[rawHash.toLowerCase()];
        if (mappedId) {
...
if (cardsContainer) {
    programs.forEach(program => {
        ...
        <div class="program-card ${program.id === 'school-of-programming' ? 'sop-card' : ''} ${program.id === 'school-of-education' ? 'soe-card' : ''} ${program.id === 'school-of-second-chance' ? 'sosc-card' : ''}" id="${program.id}">
    ...
    handleProgramHashScroll();
    window.addEventListener('hashchange', handleProgramHashScroll);
}
